...
int server_iis = 0;
int server_apache = 0;
int server_localdirectory = 0;
int verbosity = 0;
int http_port = HTTP_PORT; /* or 0, then set in main */

char cookie[64];

char directory_buf[256], auth_buf[256], info_buf[256], services_buf[256],
    directory_buf1[256];
char directory_ip[16], directory_url[256];
char prompt_string[256];
int directory_start = 0, directory_end = 0, directory_total = 0,
    structure_size = 0;
char ucm_ip[16];

struct corporate_directory *directory = NULL;
struct corporate_directory *directory_first = NULL;
struct corporate_directory *directory_hl = NULL;

// This function converts ":" Deliminated MAC address in string format.(Removes
// : delimiter) for example MAC string "00:11:22:33:44:55" into "001122334455"
void mac2string(char *mac, char *s) {
    while (*mac) {
        ...
        *s++ = *mac++;  // copy char, advance both pointers
    }
    *s = '\0';
}

int send_ack_packet(int sock, struct sockaddr_in *saddr, short block,
                    short port) {
    int send_err = 0;
    ...
    send_err = sendto(sock, &tftp_ack_packet, sizeof(struct tftp_ack), 0,
...
int recv_request_reply(int sock, struct sockaddr_in *saddr, char *file_name) {
    ...
    while (data_size ==
           512) {  // Data size less than 512 implies last data packet

        memset(tftp_recv_packet.data, '\0', sizeof(tftp_recv_packet.data));
        recv_err = recvfrom(sock, &tftp_recv_packet, sizeof(tftp_recv_packet),
                            0, (struct sockaddr *)&saddr1, &size_of);
        if (recv_err < 0) {
        ...
        if (tftp_recv_packet.opcode == htons(ERROR_CODE)) {
            ...
            /* Send ACK packet */
            int send_err = send_ack_packet(sock, saddr, tftp_recv_packet.block,
                                           saddr1.sin_port);
            if (send_err < 0) return -1;
        }
    }
    fclose(fp);
    return 0;
...
int send_tftp_request(struct tftp *tftp_packet, char *file_name, char *mode,
                      int sock, struct sockaddr_in *saddr) {
    int send_err = 0;
    ...
    send_err = sendto(
        sock, tftp_packet,
        sizeof(struct tftp) - 80 + 2 + strlen(file_name) + strlen("octet"), 0,
        (struct sockaddr *)saddr, sizeof(struct sockaddr_in));
    if (send_err < 0) {
...
void intialize_server_socket(struct sockaddr_in *saddr, char *tftp_server_ip,
                             int port) {
...
int tftp_handler(char *mac, char *tftp_server_ip) {
    ...
    /* Sends TFTP request */
    send_err =
        send_tftp_request(&tftp_packet, real_filename, mode, sock, &saddr);
    if (send_err < 0) return -1;
    ...
    /* Extracts directory, authentication, services, informational URL from
     * config file */
    int return_url = xml_config_parser(real_filename);
    if (return_url < 0) {
        printf(
            "Directory url not found in xml configuration file. Probably no "
            "corporate directory has been setup. \n");
        return -1;
    }
