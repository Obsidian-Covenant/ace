...
int xml_config_parser(char *file_name) {
    ...
    printf(
        "Successfully received file via TFTP, beginning to parse file name:  "
        "%s\n",
        file_name);
...
void parse_url(char *url) {
    char temp_buf[256] = {'\0'}, temp_buf1[256] = {'\0'},
         temp_buf2[256] = {'\0'}, port[16] = {'\0'};
    char *result = NULL;
    ...
    if (strrchr(temp_buf, ':')) {
        ...
        char delims1[] = "/";
        strncpy(temp_buf1, url + strlen("http://") + strlen(directory_ip) + 1,
                sizeof(temp_buf1) - 1);
        ...
        strncpy(
            directory_url,
            url + strlen("http://") + strlen(directory_ip) + 1 + strlen(port),
            sizeof(directory_url) - 1);
    } else {
        char delims[] = "/";
        result = strtok(temp_buf, delims);
        memset(directory_ip, '\0', sizeof(directory_ip));
        strncpy(directory_ip, result, sizeof(directory_ip) - 1);
        char delims1[] = "<";
        strncpy(temp_buf1, url + strlen("http://") + strlen(directory_ip),
                sizeof(temp_buf1) - 1);
        result = strtok(temp_buf1, delims1);
        memset(directory_url, '\0', sizeof(directory_url));
    ...
    memset(directory_ip, '\0', sizeof(directory_ip));
    strncpy(directory_ip, inet_ntoa(*(struct in_addr *)he->h_addr),
            sizeof(directory_ip) - 1);
}
...
int check_file_format(char *url) {
    ...
    if (result != NULL) {
        ...
        if (result1 != NULL) {
            ...
            if (result2 != NULL) {
                // fprintf(stdout,"Dont know the web server format, there could
                // be an error parsing the xml tags \n");
                server_localdirectory = 1;
                return -1;
            }
            fprintf(stdout,
                    "The web server uses different format, ace does not handle "
                    "the format. \n");
            return -1;
        }
    }
...
int parse_prompt() {
    ...
    if (result != NULL) {
        ...
        strcpy(prompt_buf, prompt_string + strlen("Records ") + strlen(value) +
                               strlen("to "));
        ...
        if (result1 != NULL) {
            ...
            memset(prompt_buf, '\0', sizeof(prompt_buf));
            strcpy(prompt_buf, prompt_string + strlen("Records ") +
                                   strlen(value) + strlen("to ") +
                                   strlen(value1) + strlen("of "));
