...
/* Note: Legths initialised to negative to allow us to distinguish between
 * "empty" and "not set" */
...
/*****************************************************************************/
char *cleanmetas(
    char *cstr) /* this is to clean single quotes out of DHCP strings */
{
...
void execute_on_change(char *prm) {
...
#endif
    {
        ...
        if (Cfilename)
            snprintf(exec_on_change, sizeof(exec_on_change), "%s", Cfilename);
        else
...
/*****************************************************************************/
unsigned long getgenmask(
    unsigned long ip_in) /* this is to guess genmask	*/
{
    ...
    if (ioctl(dhcpSocket, SIOCADDRT, &rtent) == -1) {
        ...
        {                         /* try adding a route to gateway first */
            ...
            if (ioctl(dhcpSocket, SIOCADDRT, &rtent) == 0) {
                ...
                rtent.rt_window = Window;
                rtent.rt_flags =
                    RTF_UP | RTF_GATEWAY | (Window ? RTF_WINDOW : 0);
                if (ioctl(dhcpSocket, SIOCADDRT, &rtent) == -1) {
...
int dhcpConfig() {
    ...
    if (ioctl(dhcpSocket, SIOCSIFBRDADDR, &ifr) ==
        -1) /* setting broadcast address */
    ...
    for (i = 0; i < DhcpOptions.len[staticRoute];
         i += 8) { /* setting static routes */
    ...
    if (SetDHCPDefaultRoutes) {
        ...
        /* rebind dhcpSocket after changing ip address to avoid problems
         * with 2.0 kernels */
#ifdef OLD_LINUX_VERSION
    ...
    if (ReplNTPConf) {
        ...
        if (f) {
            ...
            } else { /* No servers found, use local clock */
    ...
    if (SetHostName) {
		...
		if ( *dname == '.' )
		  break;
                ...
                memcpy((char *)DhcpOptions.val[hostName], hp->h_name,
                       dname_len);
        ...
        if (InitialHostName_len < 0 &&
            gethostname(InitialHostName, sizeof(InitialHostName)) == 0) {
        ...
        if (DhcpOptions.len[hostName]) {
            if (sethostname(DhcpOptions.val[hostName],
                            DhcpOptions.len[hostName]) < 0) {
                syslog(LOG_ERR, "dhcpConfig: sethostname: %m\n");
            }
            if (DebugFlag)
    ...
    if (SetDomainName) {
	  ...
	  if ( DebugFlag )
        ...
        if (!DhcpOptions.len[domainName]) {
            ...
            if (hp) {
                ...
                if (dname_len) {
                    ...
                    memcpy((char *)DhcpOptions.val[domainName], dname,
                           dname_len);
        ...
        if (DhcpOptions.len[domainName]) {
            if (setdomainname(DhcpOptions.val[domainName],
                              DhcpOptions.len[domainName]) < 0) {
                syslog(LOG_ERR, "dhcpConfig: setdomainname: %m\n");
            }
            if (DebugFlag)
                fprintf(stdout, "dhcpcd: your domainname = %s\n",
                        (char *)DhcpOptions.val[domainName]);
...
tsc:
    ...
    snprintf(hostinfo_file_old, sizeof(hostinfo_file_old), DHCP_CACHE_FILE,
             ConfigDir, IfNameExt);
    i = open(hostinfo_file_old, O_WRONLY | O_CREAT | O_TRUNC,
             S_IRUSR + S_IWUSR);
    if (i == -1 || write(i, (char *)&DhcpIface, sizeof(dhcpInterface)) == -1 ||
        close(i) == -1)
        syslog(LOG_ERR, "dhcpConfig: open/write/close: %m\n");
    snprintf(hostinfo_file, sizeof(hostinfo_file), DHCP_HOSTINFO, ConfigDir,
             IfNameExt);
...
GATEWAY=%u.%u.%u.%u",
        ...
        if (DhcpOptions.len[hostName])
            fprintf(f, "\nHOSTNAME=\'%s\'",
                    cleanmetas((char *)DhcpOptions.val[hostName]));
        if (DhcpOptions.len[domainName])
            fprintf(f, "\nDOMAIN=\'%s\'",
                    cleanmetas((char *)DhcpOptions.val[domainName]));
        if (DhcpOptions.len[nisDomainName])
            fprintf(f, "\nNISDOMAIN=\'%s\'",
                    cleanmetas((char *)DhcpOptions.val[nisDomainName]));
        if (DhcpOptions.len[rootPath])
            fprintf(f, "\nROOTPATH=\'%s\'",
                    cleanmetas((char *)DhcpOptions.val[rootPath]));
        fprintf(f,
                "\n\
DNS=%u.%u.%u.%u",
...
CLASSID=\'%s\'\n",
                ...
                ((unsigned char *)&DhcpMsgRecv->siaddr)[3], ClientHwAddr[0],
                ClientHwAddr[1], ClientHwAddr[2], ClientHwAddr[3],
                ClientHwAddr[4], ClientHwAddr[5], DhcpIface.shaddr[0],
                DhcpIface.shaddr[1], DhcpIface.shaddr[2], DhcpIface.shaddr[3],
                DhcpIface.shaddr[4], DhcpIface.shaddr[5],
                ...
                ntohl(*(unsigned int *)DhcpOptions.val[dhcpT2value]), IfNameExt,
                DhcpIface.class_id);
        if (ClientID)
