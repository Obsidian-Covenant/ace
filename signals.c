...
void sigHandler(int sig) {
    ...
    if (sig == SIGALRM) {
            ...
            if (currState == &dhcpRenew)
                ...
                if (currState == &dhcpRebind)
                    ...
                    if (currState == &dhcpReboot) {
                        ...
                        siglongjmp(
                            env, 4); /* failed to acquire the same IP address */
                    } else
                        syslog(LOG_ERR,
                               "timed out waiting for a valid DHCP server "
                               "response\n");
                }
            }
        }
        ...
        if (sig == SIGHUP) {
            dhcpRelease();
            /* allow time for final packets to be transmitted before shutting
             * down     */
            /* otherwise 2.0 drops unsent packets. fixme: find a better way than
             * sleep */
            sleep(1);
        }
