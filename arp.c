...
int arpCheck() {
    ...
    ArpMsgSend.htype =
        (TokenRingIf) ? htons(ARPHRD_IEEE802_TR) : htons(ARPHRD_ETHER);
    ...
    do {
        do {
            ...
            if (sendto(dhcpSocket, &ArpMsgSend, len, 0, &addr,
        ...
        do {
            ...
            if (ArpMsgRecv.ethhdr.ether_type != htons(ETHERTYPE_ARP)) continue;
            if (ArpMsgRecv.operation == htons(ARPOP_REPLY)) {
                if (DebugFlag)
                    syslog(LOG_DEBUG,
                           "ARPOP_REPLY received from %u.%u.%u.%u for "
                           "%u.%u.%u.%u\n",
            ...
            if (memcmp(ArpMsgRecv.tHaddr, ClientHwAddr, ETH_ALEN)) {
                if (DebugFlag)
                    syslog(LOG_DEBUG,
                           ...
                           "%02X.%02X.%02X.%02X.%02X.%02X received, "
                           "%02X.%02X.%02X.%02X.%02X.%02X expected\n",
                           ArpMsgRecv.tHaddr[0], ArpMsgRecv.tHaddr[1],
                           ArpMsgRecv.tHaddr[2], ArpMsgRecv.tHaddr[3],
                           ArpMsgRecv.tHaddr[4], ArpMsgRecv.tHaddr[5],
            ...
            if (memcmp(&ArpMsgRecv.sInaddr, &DhcpIface.ciaddr, 4)) {
                if (DebugFlag)
                    syslog(LOG_DEBUG,
                           "sender IP address mismatch: %u.%u.%u.%u received, "
                           "%u.%u.%u.%u expected\n",
                           ArpMsgRecv.sInaddr[0], ArpMsgRecv.sInaddr[1],
                           ArpMsgRecv.sInaddr[2], ArpMsgRecv.sInaddr[3],
