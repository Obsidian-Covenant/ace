...
void usage() {
    printf("ACE v1.11: Automated Corporate (Data) Enumerator\n");
    printf(
        "Usage: ace [-i interface] [ -m mac address ] [ -t tftp server ip "
        "address | -c cdp mode | -v voice vlan id | -r vlan interface | -d "
        "verbose mode ]\n\n");
    printf(
    ...
    printf(
        "Mode to automatically discover TFTP Server IP via DHCP Option 150 "
        "(-m)\n");
...
void remove_interface(char *RemovedInterface) {
    ...
    if (ioctl(fd, SIOCSIFVLAN, &if_request) < 0) {
        fprintf(stderr,
                "Error trying to remove VLAN Interface %s.  Error: %s\n",
...
int main(int argc, char *argv[]) {
    ...
    memcpy(temptcpNetworkDevice, "\\DEVICE\\TCPIP_",
           strlen("\\DEVICE\\TCPIP_"));
    temptcpNetworkDevice += strlen("\\DEVICE\\TCPIP_");
    memcpy(temptcpNetworkDevice, start, length);
    WCHAR wTCPNetworkDevice[tcpNetLength + 1];
    MultiByteToWideChar(
        CP_ACP, 0, tcpNetworkDevice, strlen(tcpNetworkDevice) + 1,
        wTCPNetworkDevice,
        sizeof(wTCPNetworkDevice) / sizeof(wTCPNetworkDevice[0]));
    ...
    if (vlan_yes == 1) {
        ...
        if (tftp_ip_yes != 1 && tftp_yes != 1) {
            printf(
                "TFTP server ip address not received in Option 150 in DHCP "
                "messages \n");
            printf(
                "See Usage, to VLAN HOP and get the TFTP server ip address \n");
            exit(1);
        }
#endif
#ifdef OS_WINDOWS
        ...
        if (!dont_set_ip) {
            printf(
                "Please wait. It will take a few seconds for setting the IP "
                "address and querying the DHCP option 150.\n");
            int retRenew = vcRenewInterfaceIP(wTCPNetworkDevice);
            if (retRenew < 0) exit(1);
        }
        int retdhcp =
            getOption150AndRouterIP(networkDevice, tftp_server_ip, router_ip);
        if (retdhcp < 0) exit(1);
        int retRoute = addRoute(wTCPNetworkDevice, tftp_server_ip, router_ip);
        if (retRoute < 0) exit(1);
#endif
    ...
    */
    ...
    /* Adding host route using popen, have to use SIOCADDRT commented above, but
     * it is not working */
...
BOOL OpenNdisDevice(HANDLE Handle, CHAR *v_NetworkDevice) {
    ...
    return (DeviceIoControl(
        Handle, IOCTL_NDISPROT_OPEN_DEVICE, (LPVOID)&wNdisDeviceName[0],
        wNameLength * sizeof(WCHAR), NULL, 0, &BytesReturned, NULL));
}
...
void windowsSetVLANID(int vlan_id, CHAR *v_NetworkDevice) {
    ...
    v_FileHandle =
        CreateFile(ndisProto, GENERIC_READ | GENERIC_WRITE,
    ...
    if (!DeviceIoControl(v_FileHandle, IOCTL_NDISPROT_BIND_WAIT, NULL, 0, NULL,
                         0, &byteReturned, NULL)) {
    ...
    if (!OpenNdisDevice(v_FileHandle, v_NetworkDevice)) {
        ...
        if (v_FileHandle) CloseHandle(v_FileHandle);
        exit(1);
    }
    ...
    if (DeviceIoControl(v_FileHandle, IOCTL_NDISPROT_QUERY_OID_VALUE,
    ...
    if (vvid == vlan_id) {
        printf(
            "The same 802.1Q tag is already set on the selected network "
            "device\n");
        dont_set_ip = 1;
        if (v_FileHandle) CloseHandle(v_FileHandle);
        return;
    ...
    if (DeviceIoControl(v_FileHandle, IOCTL_NDISPROT_SET_OID_VALUE,
                        (LPVOID)&pSetOid, sizeof(pSetOid), (LPVOID)&pSetOid,
                        sizeof(pSetOid), &byteReturned, NULL)) {
...
int getOption150AndRouterIP(CHAR *networkDevice, CHAR *Dhcp150Option,
                            CHAR *RouterIP) {
    ...
    MultiByteToWideChar(CP_ACP, 0, networkDevice, length + 1, waName,
                        sizeof(waName) / sizeof(waName[0]));
    ...
    if (!dont_parse_option150) {
        ...
        dhcpError = DhcpRequestParams(
            ...
            DHCPCAPI_REQUEST_SYNCHRONOUS, NULL, adapterName, NULL, SendParams,
            RequestParams, (PBYTE)Buffer, &bSize, NULL);
        ...
        if (DhcpOption150Params.nBytesData) {
            ...
            // 256, Option150Value[1] + 256, Option150Value[2],
            // Option150Value[3] + 256); printf("Value is %d.%d.%d.%d \n",
            ...
            sprintf(Dhcp150Option, "%d.%d.%d.%d", DhcpOption150Params.Data[0],
                    DhcpOption150Params.Data[1], DhcpOption150Params.Data[2],
                    DhcpOption150Params.Data[3]);
        } else {
            /* Windows DHCP API sucks, it is too slow and sometimes it returns
             * option 150 is not received */
            printf(
                "DHCP Option150 not found in DHCP Server. If you are using "
    ...
    DHCPCAPI_PARAMS DhcpRouterAddress = {
        ...
        0, OPTION_ROUTER_ADDRESS, FALSE, NULL, 0};
    ...
    dhcpError = DhcpRequestParams(
        ...
        DHCPCAPI_REQUEST_SYNCHRONOUS, NULL, adapterName, NULL, RouterSendParams,
        RouterRequestParams, (PBYTE)Buffer, &bSize, NULL);
    ...
    if (DhcpRouterAddress.nBytesData) {
        ...
        sprintf(RouterIP, "%d.%d.%d.%d", DhcpRouterAddress.Data[0],
                DhcpRouterAddress.Data[1], DhcpRouterAddress.Data[2],
                DhcpRouterAddress.Data[3]);
...
int addRoute(WCHAR *wTCPNetworkDevice, CHAR *Host, CHAR *RouterIP) {
    ...
    // LPWSTR adapterName =
    // L"\\Device\\TCPIP_{4979EAF3-119B-4B26-94E4-4B0F52F6FD1C}";
