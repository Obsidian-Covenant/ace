...
int dhcpclientcall(char *vinterface) {
    ...
    if (verbosity) printf("Sending DHCP request on %s\n", IfName);
...
pcap_t *create_cdp_pcap(char *IfName_temp) {
    char filter_exp[] =
        "ether host 01:00:0c:cc:cc:cc and "
        "(ether[20:2] = 0x2000 or ether[24:2] = 0x2000)";

    /* Local buffer to hold auto-selected interface name */
    char auto_ifname[PCAP_BUF_SIZE]; /* PCAP_BUF_SIZE is defined by libpcap */
    ...
    if (IfName_temp == NULL) {
        ...
        if (alldevs == NULL) {
            ...
            /* alldevs is NULL here, but freeing a NULL is harmless */
            pcap_freealldevs(alldevs);
            exit(EXIT_FAILURE);
        }

        /* take the first device name and copy it */
        strncpy(auto_ifname, alldevs->name, sizeof(auto_ifname) - 1);
        IfName_temp = auto_ifname;

        printf("Interface not specified ... %s\n", IfName_temp);

        pcap_freealldevs(alldevs);

        /* now use our own copy for the rest of the function */
        IfName_temp = auto_ifname;
    }
    ...
    if (pcap_lookupnet(IfName_temp, &net, &mask, errbuf) == -1) {
        fprintf(
            stderr,
            "Couldn't get netmask for device %s.  Enable the interface first "
            "and assign an IP address: %s\n",
            IfName_temp, errbuf);
        net = 0;
        mask = 0;
        exit(EXIT_FAILURE);
...
unsigned int mk_spoof_cdp(char *mdeviceid, char *mportid, char *msoftware,
                          char *mplatform, char *mcapas, char *mduplex) {
    ...
    */
    ...
    /* insert our address */
    /*
    cdp_ae=(struct cdp_address_entry *)((void *)cdp_addr+sizeof(struct
    cdp_address)); cdp_ae->proto_type=0x01; cdp_ae->length=0x01;
    cdp_ae->proto=0xCC;
    ...
    */
    ...
    cdp_caps =
        (struct cdp_capabilities *)((void *)cdp_prt +
                                    (sizeof(u_int16_t) + sizeof(u_int16_t) +
                                     strlen(mportid)));
    cdp_caps->type = htons(TYPE_CAPABILITIES);
    cdp_caps->length = htons(0x0008);
    cdp_caps->capab = 0;
    if (strchr(mcapas, 'R')) cdp_caps->capab = CDP_CAP_LEVEL3_ROUTER;
    if (strchr(mcapas, 'T'))
    ...
    if (strchr(mcapas, 'r')) cdp_caps->capab = cdp_caps->capab | CDP_CAP_LEVEL1;
    ...
    /* Normally the VVQ is first.  For some reason the Power Entry and VVQ had
     * to be reversed otherwise if the power bytes entry is last in the CDP
     * frame, there is a CDP checksum error.  Will get to this later when I have
     * more time
     */
    ...
    */
    ...
    cdp_dup = (struct cdp_duplex *)((void *)cdp_pow + (sizeof(u_int16_t) +
                                                       sizeof(u_int16_t) + 2));
    ...
    */
    ...
    cdp_end = (u_char *)((void *)cdp_vvq +
                         (sizeof(u_int16_t) + sizeof(u_int16_t) + 4));
    /*cdp_end=(u_char *)((void *)cdp_plt+(sizeof(u_int16_t) + sizeof(u_int16_t)
     * + strlen(mplatform)));*/
...
int get_cdp(u_char *args, const struct pcap_pkthdr *header,
            const u_char *packet) {
    ...
    if (ntohs(eth_ptr->type_length) <= 1500) {
        ...
        while (bytes_to_analyze < size_cdp_data) {
            ...
            cdpfirstfields =
                (struct CDPFirstFields *)(packet + SIZE_ETHERNET + SIZE_LLC +
                                          SIZE_CDPHEADER + cdp_toffset);
            ...
            if (cdp_type == 14) {
                ...
                voipvlanreply =
                    (struct VoIPVLANReply *)(packet + SIZE_ETHERNET + SIZE_LLC +
                                             SIZE_CDPHEADER + cdp_poffset + 1);
                vvlan_id = ntohs(voipvlanreply->voicevlan);
                printf("Discovered VoIP VLAN: %d\n", vvlan_id);

            } else {
                const u_char *cdp_data;
                cdp_data = (u_char *)(packet + SIZE_ETHERNET + SIZE_LLC +
                                      SIZE_CDPHEADER + cdp_poffset);
            }
        ...
        if (vvlan_id == 0) {
            printf(
                "Couldn't discover VoIP VLAN Reply field in CDP Packet "
                "data\n\n");
        }
        ...
        llc =
            (struct logical_link_control *)(packet + SIZE_ETHERNET + SIZE_VLAN);
        ...
        const struct CDPHeader *cdpheader;
        cdpheader =
            (struct CDPHeader *)(packet + SIZE_ETHERNET + SIZE_VLAN + SIZE_LLC);

        size_cdp_data =
            ntohs(vlan->length) - SIZE_VLAN - SIZE_LLC - SIZE_CDPHEADER;
        int bytes_to_analyze = 0;
        int cdp_toffset = 0;
        int cdp_poffset = 0;
        int cdp_flength;
        ...
        while (bytes_to_analyze < size_cdp_data) {
            const struct CDPFirstFields *cdpfirstfields;
            cdpfirstfields =
                (struct CDPFirstFields *)(packet + SIZE_ETHERNET + SIZE_VLAN +
                                          SIZE_LLC + SIZE_CDPHEADER +
                                          cdp_toffset);
            ...
            if (cdp_type == 14) {
                ...
                const struct VoIPVLANReply *voipvlanreply;
                voipvlanreply =
                    (struct VoIPVLANReply *)(packet + SIZE_ETHERNET +
                                             SIZE_VLAN + SIZE_LLC +
                                             SIZE_CDPHEADER + cdp_poffset + 1);
                vvlan_id = ntohs(voipvlanreply->voicevlan);
                printf("Discovered VoIP VLAN: %d\n", vvlan_id);
...
int cdp_mode(int mode, char *IfName_temp) {
    ...
    if (mode == 0) {
        ...
        if ((atsock = init_socket_eth(IfName_temp)) <= 0) {
            printf(
                "The interface %s must have a valid IP address in order for "
                "the CDP spoofing code to work.\nFirst set the IP address "
                "static or via DHCP, and then run again.\n",
                IfName);
        ...
        if (verbosity) {
            printf(
                "Sending 1st CDP Spoofed packet on %s with CDP packet data:\n",
                IfName_temp);
        }
        /*retval =
         * mk_spoof_cdp(S_deviceid,S_portid,S_software,S_platform,S_capas,S_duplex,S_ipaddr);*/
        retval = mk_spoof_cdp(S_deviceid, S_portid, S_software, S_platform,
                              S_capas, S_duplex);
        if (verbosity) {
        ...
        if (verbosity) {
            printf(
                "Sending 2nd CDP Spoofed packet on %s with CDP packet data:\n",
                IfName_temp);
        }
        /*retval =
         * mk_spoof_cdp(S_deviceid,S_portid,S_software,S_platform,S_capas,S_duplex,S_ipaddr);*/
        ...
        rc = pthread_create(&threads[0], NULL, send_cdp,
                            (void *)(intptr_t)atsock);
        if (rc) {
...
void *send_cdp(void *threadarg) {
    ...
    int atsock = (int)(intptr_t)threadarg;
    int retval;
    ...
    for (;;) {
        ...
        if (verbosity) {
            printf("Sent CDP packet of %d bytes\n", retval2);
            printf(
                "Sleeping for 60 seconds before sending another CDP "
                "packet\n\n");
        }
