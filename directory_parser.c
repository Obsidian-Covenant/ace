...
int alter_name_format(char *, int,
                      char *); /* Name is format as <First name> <Last name> */
int check_number_value(char *, int, char *);
int parse_tag(char *, int,
              char *); /* Parses the message for start of <DirectoryEntry> or
                          <Prompt> and returns the index */
int parse_part_tag(char *, int, char *); /* Parses the remaining part of the tag
                                            value from the next message */
int parse_prompt_value(char *, int); /* Parses the prompt tag values and copies
                                        it to (prompt_string) buffer */
int parse_prompt_value_part(char *, int); /* Parses the remanining part of the
                                             tag value from the next message */
...
void print_corporate_data() {
    ...
    for (i = 0; i < structure_size; i++) {
        ...
        int return_value = alter_name_format(
            directory[i].name, sizeof(directory[i].name), new_name);
        if (return_value < 0) {
            fprintf(stderr,
                    "First name and last name cannot be differentiated, "
                    "skipping the name %s \n",
                    directory[i].name);
        } else {
            // printf("Retrieved Name: %s\tNumber:
...
int parse_prompt_value_part(char *data, int new_pointer) {
    ...
    strncpy(temp_buf + strlen(temp_buf), buffer,
            sizeof(temp_buf) - strlen(temp_buf) - 1);
    memset(prompt_string, '\0', sizeof(prompt_string));
    strncpy(prompt_string, temp_buf, sizeof(prompt_string) - 1);
    memset(temp_buf, '\0', sizeof(temp_buf));
...
int parse_directory_end_tag_part(char *data, int new_pointer) {
    ...
    if (read_char == '>') {
        ...
        strncpy(directory_hl->name, directory_name,
                sizeof(directory_hl->name) - 1);
        strncpy(directory_hl->telephone_number, directory_number,
                sizeof(directory_hl->telephone_number) - 1);
    ...
    strncpy(directory_hl->telephone_number, directory_number,
            sizeof(directory_hl->telephone_number));
    structure_size = structure_size + 1;
...
int parse_number_value_part(char *data, int new_pointer) {
    ...
    if (read_char == '<') {
        ...
        directory_first = directory_hl;
        strncpy(directory_hl->name, directory_name, sizeof(directory_hl->name));
        strncpy(directory_hl->telephone_number, number_value,
                sizeof(directory_hl->telephone_number));
        structure_size = structure_size + 1;
...
int parse_number_start_tag_part(char *data, int new_pointer) {
    ...
    if (read_char == '>') {
        ...
        directory_first = directory_hl;
        strncpy(directory_hl->name, directory_name, sizeof(directory_hl->name));
        strncpy(directory_hl->telephone_number, number_buffer,
                sizeof(directory_hl->telephone_number));
        structure_size = structure_size + 1;
...
int parse_name_end_tag_part(char *data, int new_pointer) {
    ...
    if (read_char == '>') {
        ...
        new_pointer =
            parse_starting_tag(data, ++i, string, &number_start_tag_part);
        if (new_pointer < 0) return -1;
...
int parse_name_value_part(char *data, int new_pointer) {
    ...
    if (read_char == '<') {
        ...
        memset(string, '\0', sizeof(string));
        strncpy(string, "Telephone", sizeof(string) - 1);
        new_pointer = parse_starting_tag(data, new_pointer, string,
                                         &number_start_tag_part);
        if (new_pointer < 0) return -1;
    ...
    strncpy(string, "Telephone", sizeof(string) - 1);
    new_pointer =
        parse_starting_tag(data, new_pointer, string, &number_start_tag_part);
    if (new_pointer < 0) {
        if (verbosity) {
            printf("Error at parsing number start tag\n");
        }
...
int parse_name_start_tag_part(char *data, int new_pointer) {
    ...
    new_pointer = check_name_tag_end(data, new_pointer);
    if (new_pointer < 0) {
        if (verbosity) printf("Error at parsing name end tag\n");
        return -1;
    }
    memset(string, '\0', sizeof(string));
    strncpy(string, "Telephone", sizeof(string) - 1);
    new_pointer =
        parse_starting_tag(data, new_pointer, string, &number_start_tag_part);
    if (new_pointer < 0) {
        if (verbosity) printf("Error at parsing number start tag\n");
        return -1;
    }
    new_pointer = check_number_value(data, new_pointer, number_buffer);
    if (new_pointer < 0) {
        if (verbosity) printf("Error at parsing number value\n");
        return -1;
    }

    new_pointer = check_number_tag_end(data, new_pointer);
    if (new_pointer < 0) {
        if (verbosity) printf("Error at parsing number end tag\n");
        return -1;
    }
    new_pointer = check_directory_end(data, new_pointer);
    if (new_pointer < 0) {
        if (verbosity) printf("Error at parsing directory end\n");
        return -1;
    }
...
int parse_directory_part(char *data, int new_pointer) {
    ...
    if (read_char == '>') {
        ...
        new_pointer =
            parse_starting_tag(data, ++i, string, &name_start_tag_part);
        if (new_pointer < 0) return -1;
    ...
    if (new_pointer < 0) {
        if (verbosity) printf("Error parsing name tag\n");
        return -1;
    }
    new_pointer = check_name_value(data, new_pointer, name_buffer);
    if (new_pointer < 0) {
        if (verbosity) printf("Error at parsing name value\n");
        return -1;
    }
    new_pointer = check_name_tag_end(data, new_pointer);
    if (new_pointer < 0) {
        ...
        if (strcmp(string, "DirectoryEntry") == 0) {
            ...
            if (new_pointer < 0) {
                if (name_start_tag_part == 1) {
                    if (verbosity) printf("Part of name start tag is parsed\n");
                } else {
                    if (verbosity) printf("Error parsing name start tag\n");
                }
                end_of_flag = 0;
                break;
            }
            ...
            if (new_pointer < 0) {
                if (name_value_part == 1) {
                    if (verbosity) printf("Part of name value is parsed\n");
                }

                else {
                    if (verbosity) printf("Error parsing name value\n");
                }
                end_of_flag = 0;
                break;
            }
            ...
            if (new_pointer < 0) {
                if (name_end_tag_part == 1) {
                    ...
                    if (verbosity) printf("Error parsing named end tag\n");
                }
            ...
            if (new_pointer < 0) {
                if (number_start_tag_part == 1) {
                    ...
                    if (verbosity) printf("Error parsing number start tag\n");
                }
                memset(directory_name, '\0', sizeof(directory_name));
                strncpy(directory_name, name_buffer,
                        sizeof(directory_name) - 1);
                end_of_flag = 0;
                break;
            }
            new_pointer = check_number_value(data, new_pointer, number_buffer);
            if (new_pointer < 0) {
                if (number_value_part == 1) {
                    ...
                    if (verbosity) printf("Error: parsing number value \n");
                }
                memset(directory_name, '\0', sizeof(directory_name));
                strncpy(directory_name, name_buffer,
                        sizeof(directory_name) - 1);
            ...
            if (new_pointer < 0) {
                if (number_end_tag_part == 1) {
                    ...
                    if (verbosity) printf("Error: Parsing number end tag \n");
                }
                memset(directory_name, '\0', sizeof(directory_name));
                memset(directory_number, '\0', sizeof(directory_number));
                strncpy(directory_name, name_buffer,
                        sizeof(directory_name) - 1);
                strncpy(directory_number, number_buffer,
                        sizeof(directory_number) - 1);
                end_of_flag = 0;
                break;
            }
            new_pointer = check_directory_end(data, new_pointer);
            if (new_pointer < 0) {
                if (directory_end_part == 1) {
                    ...
                    if (verbosity) printf("Error:Parsing directory end tag \n");
                }
                memset(directory_name, '\0', sizeof(directory_name));
                memset(directory_number, '\0', sizeof(directory_number));
                strncpy(directory_name, name_buffer,
            ...
            directory_hl->next = directory_first;
            directory_first = directory_hl;
            strncpy(directory_hl->name, name_buffer,
                    sizeof(directory_hl->name) - 1);
            strncpy(directory_hl->telephone_number, number_buffer,
                    sizeof(directory_hl->telephone_number) - 1);
...
int parse_localdirectory(char *data) {
    ...
    if (new_pointer < 0) {
        if (verbosity) printf("Error: Parsing CiscoIPPhoneDirectory \n");
    }
...
int parse_first_recv(char *data) {
    ...
    if (new_pointer < 0) {
        if (verbosity) printf("Error: Parsing xml version \n");
        new_pointer = 0;
    }
...
int parse_other_recv(char *data) {
    ...
    if (prompt_value_part == 1) {
        ...
        if (new_pointer < 0) {
            if (verbosity) printf("Error: Parsing directory part \n");
            return -1;
        }
        ...
        if (new_pointer < 0) {
            if (verbosity) printf("Error: Parsing name part \n");
            return -1;
        }
        ...
        if (new_pointer < 0) {
            if (verbosity) printf("Error: Parsing name value part \n");
            return -1;
        }
        name_value_part = 0;
        int retval = parse_directory_data(data, new_pointer);
        ...
        if (new_pointer < 0) {
            if (verbosity) printf("Error: Parsing name end part \n");
            return -1;
        }
        name_end_tag_part = 0;
        int retval = parse_directory_data(data, new_pointer);
        ...
        if (new_pointer < 0) {
            if (verbosity) printf("Error: Parsing number start tag part \n");
            return -1;
        }
        ...
        if (new_pointer < 0) {
            if (verbosity) printf("Error: Parsing number value part \n");
            return -1;
        }
        number_value_part = 0;
        int retval = parse_directory_data(data, new_pointer);
        ...
        if (new_pointer < 0) {
            if (verbosity) printf("Error: Parsing number end tag part \n");
            return -1;
        }
        ...
        if (new_pointer < 0) {
            if (verbosity) printf("Error: Parsing directory end tag part \n");
            return -1;
        }
...
int parse_first_two_messages(char *data) {
    ...
    new_pointer = parse_starting_tag(data, new_pointer, string, &temp);
    if (new_pointer < 0) {
        if (verbosity) printf("Error: Parsing CiscoIPPhoneMenu \n");
        return -1;
    }
    ...
    new_pointer = check_prompt(data, new_pointer);
    if (new_pointer < 0) {
        if (verbosity) printf("Error Parsing Prompt \n");
        // return -1;
        noPrompt = 1;
    }
    if (!noPrompt) {
    ...
    new_pointer = parse_starting_tag(data, new_pointer, string, &temp);
    if (new_pointer < 0) {
        if (verbosity) printf("Error Parsing MenuItem starting tag \n");
        return -1;
    }
    ...
    if (strcmp(name_buffer, "Personal Directory") == 0) {
        new_pointer = check_name_tag_end(data, new_pointer);
        if (new_pointer < 0) {
            if (verbosity) printf("Error parsing name tag end \n");
            return -1;
        }
        memset(string, '\0', sizeof(string));
        ...
        new_pointer = parse_starting_tag(data, new_pointer, string, &temp);
        if (new_pointer < 0) {
            if (verbosity) printf("Error parsing url start tag \n");
            return -1;
        }
        ...
        new_pointer = check_name_value(data, new_pointer, name_buffer);
        if (new_pointer < 0) {
            if (verbosity) printf("Error parsing url value \n");
            return -1;
        }

        if (verbosity) printf("Personal Directory URL:  %s\n", name_buffer);

        new_pointer = check_url_end_tag(data, new_pointer);
        if (new_pointer < 0) {
            if (verbosity) printf("Error: Parsing URL end tag \n");
            return -1;
        }

        new_pointer = check_menu_item_end_tag(data, new_pointer);
        if (new_pointer < 0) {
            if (verbosity) printf("Error: Parsing MenuItem end tag\n");
            return -1;
        }
        ...
        new_pointer = parse_starting_tag(data, new_pointer, string, &temp);
        if (new_pointer < 0) {
            if (verbosity) printf("Error:Parsing MenuItem start tag \n");
            return -1;
        }
        ...
        if (strcmp(name_buffer, "Corporate Directory") == 0) {
            new_pointer = check_name_tag_end(data, new_pointer);
            if (new_pointer < 0) {
                if (verbosity) printf("Error: Parsing name end tag \n");
                return -1;
            }
            ...
            new_pointer = check_menu_item_end_tag(data, new_pointer);
            if (new_pointer < 0) {
                if (verbosity) printf("Error parsing menu item end tag \n");
                return -1;
            }
        }
