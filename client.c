...
extern char InitialHostName[HOSTNAME_MAX_LEN];
extern int InitialHostName_len;
extern char InitialDomainName[HOSTNAME_MAX_LEN];
...
const struct ip *ipSend =
    (struct ip *)((struct udpiphdr *)UdpIpMsgSend.udpipmsg)->ip;
const struct ip *ipRecv =
    (struct ip *)((struct udpiphdr *)UdpIpMsgRecv.udpipmsg)->ip;
const dhcpMessage *DhcpMsgSend =
    (dhcpMessage *)&UdpIpMsgSend.udpipmsg[sizeof(udpiphdr)];
dhcpMessage *DhcpMsgRecv =
    (dhcpMessage *)&UdpIpMsgRecv.udpipmsg[sizeof(udpiphdr)];
        ...
        if (DhcpOptions.val[i]) switch (i) {
                ...
                case 53: /* dhcpMessageType */
                    fprintf(stderr, "i=%-2d  len=%-2d  option = %u\n", i,
                            DhcpOptions.len[i],
                            *(unsigned char *)DhcpOptions.val[i]);
                    break;
                default:
                    fprintf(stderr, "i=%-2d  len=%-2d  option = \"%s\"\n", i,
                            DhcpOptions.len[i], (char *)DhcpOptions.val[i]);
            }
    fprintf(
        stderr,
        "\
...
ServerHardwareAddr   = %02X.%02X.%02X.%02X.%02X.%02X\n",
        ...
        ((unsigned char *)&DhcpMsgRecv->giaddr)[3], DhcpMsgRecv->sname,
        UdpIpMsgRecv.ethhdr.ether_shost[0], UdpIpMsgRecv.ethhdr.ether_shost[1],
        UdpIpMsgRecv.ethhdr.ether_shost[2], UdpIpMsgRecv.ethhdr.ether_shost[3],
        UdpIpMsgRecv.ethhdr.ether_shost[4], UdpIpMsgRecv.ethhdr.ether_shost[5]);
#endif
    if (!DhcpMsgRecv->yiaddr) DhcpMsgRecv->yiaddr = DhcpMsgSend->ciaddr;
    if (!DhcpOptions
             .val[dhcpServerIdentifier]) /* did not get dhcpServerIdentifier */
    { /* make it the same as IP address of the sender */
        DhcpOptions.val[dhcpServerIdentifier] = malloc(4);
        memcpy(DhcpOptions.val[dhcpServerIdentifier], &ipRecv->ip_src.s_addr,
               4);
        DhcpOptions.len[dhcpServerIdentifier] = 4;
        DhcpOptions.num++;
        if (DebugFlag)
            syslog(LOG_DEBUG,
                   "dhcpServerIdentifier option is missing in DHCP server "
                   "response. Assuming %u.%u.%u.%u\n",
    ...
    if (DhcpOptions.val[dhcpServerIdentifier]) {
        ...
        snprintf(router_ip, sizeof(router_ip), "%u.%u.%u.%u",
                 ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[0],
                 ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[1],
                 ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[2],
                 ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[3]);
    ...
    if (DhcpOptions.val[dhcpTftpServerIp]) {
        ...
        if (dont_parse_option150 != 1) {
            snprintf(tftp_server_ip, sizeof(tftp_server_ip), "%u.%u.%u.%u",
                     ((unsigned char *)DhcpOptions.val[dhcpTftpServerIp])[0],
                     ((unsigned char *)DhcpOptions.val[dhcpTftpServerIp])[1],
                     ((unsigned char *)DhcpOptions.val[dhcpTftpServerIp])[2],
                     ((unsigned char *)DhcpOptions.val[dhcpTftpServerIp])[3]);
    ...
    {                          /* make it the same as dhcpServerIdentifier */
        ...
        if (DebugFlag)
            syslog(LOG_DEBUG,
                   "dns option is missing in DHCP server response. Assuming "
                   "%u.%u.%u.%u\n",
    ...
    if (!DhcpOptions.val[subnetMask]) /* did not get subnetMask */
    {
	  ...
	  if ( ((unsigned char *)&DhcpMsgRecv->yiaddr)[0] < 192 )
        ...
        if (DebugFlag)
            syslog(LOG_DEBUG,
                   "subnetMask option is missing in DHCP server response. "
                   "Assuming %u.%u.%u.%u\n",
    ...
    if (!DhcpOptions.val[broadcastAddr]) /* did not get broadcastAddr */
    {
        ...
        if (DebugFlag)
            syslog(LOG_DEBUG,
                   "broadcastAddr option is missing in DHCP server response. "
                   "Assuming %u.%u.%u.%u\n",
  ...
  if ( ! DhcpOptions.val[routersOnSubnet] )
    ...
    if (DhcpOptions.val[dhcpIPaddrLeaseTime] &&
        ...
        if (*(unsigned int *)DhcpOptions.val[dhcpIPaddrLeaseTime] == 0) {
            ...
            if (DebugFlag)
                syslog(LOG_DEBUG,
                       "dhcpIPaddrLeaseTime=0 in DHCP server response. "
                       "Assuming %u sec\n",
                       LeaseTime);
        } else if (DebugFlag)
            syslog(
                LOG_DEBUG, "dhcpIPaddrLeaseTime=%u in DHCP server response.\n",
    ...
    } else /* did not get dhcpIPaddrLeaseTime */
    {
        ...
        if (DebugFlag)
            syslog(LOG_DEBUG,
                   "dhcpIPaddrLeaseTime option is missing in DHCP server "
                   "response. Assuming %u sec\n",
                   LeaseTime);
    }
    ...
    {
        unsigned t2 =
            0.5 * ntohl(*(unsigned int *)DhcpOptions.val[dhcpIPaddrLeaseTime]);
        int t1 = htonl(t2);
        ...
        if (DebugFlag)
            syslog(LOG_DEBUG,
                   "dhcpT1value is missing in DHCP server response. Assuming "
                   "%u sec\n",
                   t2);
    }
    if (DhcpOptions.val[dhcpT2value] && DhcpOptions.len[dhcpT2value] == 4) {
        if (*(unsigned int *)DhcpOptions.val[dhcpT2value] == 0) {
            ...
            if (DebugFlag)
                syslog(LOG_DEBUG,
                       "dhcpT2value is missing in DHCP server response. "
                       "Assuming %u sec\n",
                       t2);
        }
    } else /* did not get T2 */
    {
        ...
        if (DebugFlag)
            syslog(LOG_DEBUG,
                   "dhcpT2value is missing in DHCP server response. Assuming "
                   "%u sec\n",
                   t2);
    }
...
void classIDsetup() {
    ...
    DhcpIface.class_len =
        snprintf(DhcpIface.class_id, CLASS_ID_MAX_LEN, "%s %s %s",
...
void clientIDsetup() {
    ...
    *c++ = (TokenRingIf) ? ARPHRD_IEEE802_TR
                         : ARPHRD_ETHER; /* type: Ethernet address */
#if 0
...
int eth2tr(struct packed_ether_header *frame, int datalen) {
    struct trh_hdr *phdr;
    struct trllc *pllc;
    char trheader[sizeof(struct trh_hdr) - sizeof(phdr->rseg) +
                  sizeof(struct trllc)];
    int len;
    ...
    if (phdr->daddr[0] & 0x80) { /* Destination is a broadcast */
        phdr->rcf = sizeof(phdr->rcf) |
                    htons(TR_RCF_BROADCAST | 0x70); /* Unlimited frame length */
...
int tr2eth(struct packed_ether_header *frame) {
...
#endif
        if (hlen < sizeof(hdr.rcf) || (hlen & 1)) return 1;
        hdr.saddr[0] &= ~TR_RII;
    }
    pllc = (struct trllc *)(((__u8 *)frame) + sizeof(struct trh_hdr) -
                            TR_MAXRIFLEN + hlen);
    if (pllc->dsap != EXTENDED_SAP || pllc->llc != UI_CMD) {
        if (hlen == 0) goto fake_rif; /* Bug in 2.2.3 kernel */
#ifdef DEBUG
    ...
    memmove(frame + 1, pllc + 1,
            IPPACKET_SIZE); /* Move data portion: Overlapping buffer */
#ifdef DEBUG
...
/*****************************************************************************/
int dhcpSendAndRecv(unsigned xid, int msg, void (*buildDhcpMsg)(unsigned)) {
    ...
    do {
        do {
            ...
            if (TokenRingIf) /* Here we convert a Eth frame into an TR frame */
                len = eth2tr(&UdpIpMsgSend.ethhdr,
                             sizeof(udpiphdr) + sizeof(dhcpMessage));
            else
                len = sizeof(struct packed_ether_header) + sizeof(udpiphdr) +
                      sizeof(dhcpMessage);
            if (sendto(dhcpSocket, &UdpIpMsgSend, len, 0, &addr,
        ...
        do {
            ...
            if (TokenRingIf) { /* Here we convert a TR frame into an Eth frame
                                */
            ...
            timeout =
                j - diff.tv_sec * 1000000 - diff.tv_usec + random() % 200000;
            if (UdpIpMsgRecv.ethhdr.ether_type != htons(ETHERTYPE_IP)) continue;
            ...
            memcpy(&ipRecv_local, ((struct udpiphdr *)tmp_ip)->ip,
                   sizeof(struct ip));
            udpRecv =
                (struct udphdr *)((char *)(((struct udpiphdr *)tmp_ip)->ip) +
                                  sizeof(struct ip));
            ...
            if (len < i) {
                if (DebugFlag)
                    syslog(LOG_DEBUG,
                           "corrupted IP packet of size=%d and ip_len=%d "
                           "discarded\n",
                           len, i);
                continue;
            }
            ...
            if (len < i) {
                if (DebugFlag)
                    syslog(LOG_DEBUG,
                           "corrupted UDP msg of size=%d and uh_ulen=%d "
                           "discarded\n",
                           len, i);
                continue;
            }
            if (DoCheckSum) {
                ...
                if (len) {
                    if (DebugFlag) switch (len) {
                            case -1:
                                syslog(LOG_DEBUG,
                                       "corrupted IP packet with ip_len=%d "
                                       "discarded\n",
                                       (int)ntohs(ipRecv_local.ip_len));
                                break;
                            case -2:
                                syslog(LOG_DEBUG,
                                       "corrupted UDP msg with uh_ulen=%d "
                                       "discarded\n",
            ...
            while (udpFooSocket > 0 &&
                   recvfrom(udpFooSocket, (void *)foobuf, sizeof(foobuf), 0,
                            NULL, NULL) != -1);
            if (parseDhcpMsgRecv() == msg) return 0;
            if (DhcpOptions.val[dhcpMessageType])
                if (*(unsigned char *)DhcpOptions.val[dhcpMessageType] ==
                    DHCP_NAK) {
...
void *dhcpStart() {
    ...
    ifr.ifr_flags =
        saved_if_flags | IFF_UP | IFF_BROADCAST | IFF_NOTRAILERS | IFF_RUNNING;
    ...
    do {
        ...
        if (i > 1)
            syslog(LOG_WARNING,
                   "dhcpStart: retrying MAC address request "
        ...
        if (setsockopt(dhcpSocket, SOL_SOCKET, SO_BROADCAST, &o, sizeof(o)) ==
            -1) {
            syslog(LOG_ERR, "dhcpStart: setsockopt: %m\n");
            exit(1);
        }
        ifr.ifr_flags = saved_if_flags | IFF_UP | IFF_BROADCAST |
        ...
        if (DebugFlag)
            fprintf(stdout,
                    ...
                    ClientHwAddr[0], ClientHwAddr[1], ClientHwAddr[2],
                    ClientHwAddr[3], ClientHwAddr[4], ClientHwAddr[5]);
    } while (!ClientHwAddr[0] && !ClientHwAddr[1] && !ClientHwAddr[2] &&
             !ClientHwAddr[3] && !ClientHwAddr[4] && !ClientHwAddr[5] &&
             i < HWADDR_TRIES);

    i = time(NULL) + ClientHwAddr[5] + 4 * ClientHwAddr[4] +
        8 * ClientHwAddr[3] + 16 * ClientHwAddr[2] + 32 * ClientHwAddr[1] +
        64 * ClientHwAddr[0];
    srandom(i);
    ip_id = i & 0xffff;
    ...
    if (bind(udpFooSocket, (struct sockaddr *)&clientAddr,
             sizeof(clientAddr))) {
        if (errno != EADDRINUSE) syslog(LOG_ERR, "dhcpStart: bind: %m\n");
        close(udpFooSocket);
        udpFooSocket = -1;
...
ClientID = \"%u.%u.%u.%02X.%02X.%02X.%02X.%02X.%02X\"\n",
            DhcpIface.class_id, DhcpIface.client_id[0], DhcpIface.client_id[1],
...
void *dhcpRelease() {
    ...
    if (sendto(dhcpSocket, &UdpIpMsgSend,
               sizeof(struct packed_ether_header) + sizeof(udpiphdr) +
                   sizeof(dhcpMessage),
               0, &addr, sizeof(struct sockaddr)) == -1)
        syslog(LOG_ERR, "dhcpRelease: sendto: %m\n");
    arpRelease(); /* clear ARP cache entries for client IP addr */
    if (SetHostName) {
        if (sethostname(InitialHostName, InitialHostName_len) == -1) {
            syslog(LOG_ERR, "dhcpRelease: sethostname: %m\n");
        }
        if (DebugFlag)
            fprintf(stdout, "dhcpcd: your hostname = %s\n", InitialHostName);
    }
    if (SetDomainName) {
        if (setdomainname(InitialDomainName, InitialDomainName_len) == -1) {
            syslog(LOG_ERR, "dhcpRelease: setdomainname: %m\n");
        }
        if (DebugFlag)
...
void *dhcpStop() {
    ...
    tsc:
        close(dhcpSocket);
        if (resolv_renamed) rename("" RESOLV_CONF ".sv", RESOLV_CONF);
        if (yp_renamed) rename("" NIS_CONF ".sv", NIS_CONF);
        if (ntp_renamed) rename("" NTP_CONF ".sv", NTP_CONF);
